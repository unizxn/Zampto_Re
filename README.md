...
例如：
```
https://api.github.com/repos/zhangsan/zampto-renew/dispatches
```
...
**Request Headers（点击 Add Header 逐条添加）：**
...
```json
{
  ...
  "client_payload": {
    "reason": "Uptime Kuma 检测到服务器端口不可达",
    "status": "down",
    "heartbeat": {}
  }
}
...
5. 点击 **Test** 测试是否能成功触发 GitHub Actions
6. 点击 **Save**
...
> ⚠️ **重要：** 脚本目前在截图中**不会隐藏**页面上显示的账号信息（如邮箱地址）。

截图会上传到 GitHub Actions Artifacts，默认仅仓库成员可见。但仍建议注意以下几点：

**风险点：**
- 截图文件 `01_login_success.png` 等会保存登录后的页面，页面上可能显示你的邮箱地址
- Artifacts 默认保留 3 天，任何有仓库访问权限的人都能下载

**建议措施：**
1. **将仓库设为私有**（最重要，防止外部访问）
2. 减少 Artifact 保留天数（当前已设为 3 天）
3. 如需彻底规避，可在 `take_screenshot` 函数中加入遮罩处理，或在非敏感页面时再截图

**临时处理：** 如果截图已经上传且包含敏感信息，可以在 Actions → 对应运行记录 → Artifacts 右侧点 **Delete** 手动删除。
...
**Q: 登录失败怎么办？**
A: 检查 `ZAMPTO_USERNAME` 和 `ZAMPTO_PASSWORD` 是否正确，Zampto 的用户名通常是 `ssd000xxxxx` 格式而非邮箱。
